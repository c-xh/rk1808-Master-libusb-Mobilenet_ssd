...
/**
 * Created by cxh on 2019/1/25
 * E-mail: shon.chen@rock-chips.com
 */
public class CheckUtil {//检测service是否在运行
    ...
    public static boolean isRunningProcess(Context context, String appInfo) {
        ActivityManager myManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningAppProcessInfo> runningAppPs = null;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.CUPCAKE) {
            runningAppPs = myManager.getRunningAppProcesses();
        }
